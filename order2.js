function Submit(classid) { 
    ...
    var determinantB
    determinantB = (this.matrixB[0][0]*this.matrixB[1][1])-(this.matrixB[0][1]*this.matrixB[1][0]);
    ...
    if (this.AxDimension!=1) {
        for (var i =0; i<this.AxDimension; i++) {
            for (var j=0; j<this.AyDimension; j++) {
                result[i][j]=Math.round(result[i][j]/this.determinantB*100)/100;
            }
        }
    }
    var result2 = [];
    ...
    for (i=0; i<rowsRes; i++) {
        for (j=0; j<columnsRes; j++) {
            result2[i][j] = this.matrixA[i][0]*result[0][j]+this.matrixA[i][1]*result[1][j];
